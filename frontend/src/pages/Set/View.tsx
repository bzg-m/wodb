...
export function ViewPage(): preact.JSX.Element {
    ...
    const renderedObjects = set.objects.map((o, idx) => {
        ...
        return (
            ...
            >
                <span class={'absolute ' + badgePos + ' bg-white text-xs font-bold rounded-full w-6 h-6 flex items-center justify-center border'}>{labelForIndex(idx)}</span>
                {o.type === 'image' ? (
                        ...
                        <ul>
                            {visibleAnnotations.map((a) => (
                                <li key={a.id} class="mb-2 border p-2">
                                    ...
                                    <div class="text-sm text-gray-600">Object: {labelForObjectId(set, a.objectId)}</div>
